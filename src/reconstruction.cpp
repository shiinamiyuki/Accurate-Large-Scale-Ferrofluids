...
void reconstruct(Eigen::MatrixXd &V, Eigen::MatrixXi &F, const Eigen::MatrixXd &P, const Eigen::Vector3i &res,
    ...
    Eigen::Vector3d lower = V.colwise().minCoeff();
    Eigen::Vector3d upper = V.colwise().maxCoeff();
    Eigen::Vector3d extent = upper - lower;
    Eigen::VectorXd S;
    Eigen::MatrixXd GV;
    S.resize(res.prod());
    GV.resize(res.prod(), 3);
    Eigen::Vector3d cell_size = extent.array() / res.cast<double>().array();
    ...
    std::vector<Eigen::Matrix3d, Eigen::aligned_allocator<Eigen::Matrix3d>> G(GV.rows());
    std::vector<std::vector<int>> neighbors(P.rows());
    Eigen::MatrixXd X;
    X.resizeLike(GV);
    auto get_cell_index = [&](const Eigen::Vector3d &x) -> Eigen::Vector3i {
        return Eigen::Vector3i(((x - lower).array() / extent.array() * res.array().cast<double>()).cast<int>());
    };
    ...
    for (int i = 0; i < GV.rows(); i++) {
        ...
        for (auto j : neighbors[i]) {
            ...
            auto k = 10. / (7. * igl::PI * h * h);
            auto W = k * G[i].norm() * kernel_P((G[i] * r).norm(), h);
